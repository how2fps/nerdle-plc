...
int validate_equation(const char *expr)
{
    ...
    int count = 0;           /* tracks character count (must == 8) */
    int equal_position = -1; /* position of '=' in the string */
    int i;
    double lhs_val = 0.0;
    double rhs_val = 0.0;
    ...
    /* Must end in S4 which is the accepting state */
    ...
    /* Ensure '=' is not at the very start or very end */
    ...
    /* Split on '=' and evaluate lhs and rhs */
